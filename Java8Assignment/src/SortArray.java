...
public class SortArray {
	...
	public static void main(String[] args) {
		...
		Collections.sort(course_List,Comparator.comparing(Course::getTotal_Fees).reversed().
				thenComparing(Course::getCourse_Duration));
		//course_List.forEach(System.out::println);
		
		Collections.sort(course_List,Comparator.comparing(Course::getTotal_Fees));
		//course_List.forEach(System.out::println);
		
		 course_List.stream().sorted(Comparator.comparing(Course::getTotal_Fees)).forEach(System.out::println);;
		
		
		List<Integer> sort = course_List.stream().map(i->i.getTotal_Fees()).sorted().collect(Collectors.toList());
